...
(lambda pack! [identifier ?options]
  "Declares a plugin with its options. This macro adds it to the ed/pack
  global table to later be used in the `unpack!` macro.
  See https://github.com/wbthomason/packer.nvim for information about the
  options."
  (assert-compile (str? identifier) "expected string for identifier" identifier)
  (if (not (nil? ?options)) (assert-compile (tbl? ?options) "expected table for options" ?options))
  (table.insert _G.ed/pack (pack identifier ?options)))

(lambda rock! [identifier ?options]
  "Declares a rock with its options. This macro adds it to the ed/rock
  global table to later be used in the `unpack!` macro.
  See https://github.com/wbthomason/packer.nvim for information about the
  options."
  (assert-compile (str? identifier) "expected string for identifier" identifier)
  (if (not (nil? ?options)) (assert-compile (tbl? ?options) "expected table for options" ?options))
  (table.insert _G.ed/rock (rock identifier ?options)))

(lambda unpack! []
  "Initializes the plugin manager with the plugins previously declared and
  their respective options."
  (let [packs (icollect [_ v (ipairs _G.ed/pack)] `(use ,v))
        rocks (icollect [_ v (ipairs _G.ed/rock)] `(use_rocks ,v))
        use-sym (sym :use)]
    (tset _G :ed/pack [])
    (tset _G :ed/rock [])
